...
start = time.time()
path = '/Users/derrickchen/Documents/圆心科技/project_2/'
first_file = path + 'file_1'
second_file = path + 'file_2'
...
if len(ff) or len(sf) > 100:
    ...
    for i in range(len(ff_list)):
        ...
        second_chunk = path + 'file_temp_sf_{}.txt'.format(i)
        if i == 0:
            ...
            for line in tbody_list[1 : -1]:
                ...
                no = re.search('id = "from(\d*)_' + str(n) + '">' + str(n) + '<', line)#.group(1)
        ...
        print("Generating difference_" + str(i))
